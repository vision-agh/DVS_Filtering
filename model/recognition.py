...
class LNRecognition(L.LightningModule):
    def __init__(self, cfg_dataset, cfg_model):
        ...
        if cfg_dataset.representation.type == 'event_frame':
            ...
            self.model = MaxVitModel(cfg_model, num_classes=self.num_classes)
            # self.model = MaxViT(
            #     num_classes = self.num_classes,
            #     dim_conv_stem = cfg_model.dim_conv_stem,               # dimension of the convolutional stem, would default to dimension of first layer if not specified
            #     dim = cfg_model.dim,                         # dimension of first layer, doubles every layer
            #     dim_head = cfg_model.dim_head,                    # dimension of attention heads, kept at 32 in paper
            #     depth = (1, 1, 1, 1),             # number of MaxViT blocks per stage, which consists of MBConv, block-like attention, grid-like attention
            #     window_size = cfg_model.window_size,                  # window size for block and grids
            #     mbconv_expansion_rate = cfg_model.mbconv_expansion_rate,        # expansion rate of MBConv
            #     mbconv_shrinkage_rate = cfg_model.mbconv_shrinkage_rate,     # shrinkage rate of squeeze-excitation in MBConv
            #     dropout = cfg_model.dropout,                     # dropout
            #     channels = cfg_model.channels,                      # number of input channels
            #     )
